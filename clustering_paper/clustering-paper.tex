...
association. To take into account this feature, an integer parameter
...
the the spatial relationship between hotspots. An parameter \(adjDist\)
...
function is a \texttt{spotoroo} object which is not handful to be used
...
When using this function, if you would like to keep all information from
the clustering result including noise points, set
\texttt{noise\ =\ TRUE}.
...
ignition locations showing the spatial distribution of the fires.

According to the plot, there are total of 6 clusters where 4 of them are
at the bottom right of the plot and 2 of them are at the top left of the
plot. They were all observed from December, 2019 to Feburary, 2020 which
was during the 2019-2020 Australian bushfire season. Furthermore, the
black dots are the ignition location of each of the cluster.
...
\texttt{step} controls the time difference between successive step and
using small value of \texttt{step} will produce complex path. Moreover,
...
point. It shows that fire 1, 4 and 5 moved toward the south east, fire 2
and 6 moved toward the north west and fire 3 moved toward the south
west.
...
hotspots belong to different clusters and the green wave is the density
plot for all the hotspots. Besides, the orange dots are noise. From this
plot, we can say that most of the hotspots were observed in the
mid-January.
...
\protect\hyperlink{paremeter-tuning}{Paremeter tuning}.
...
Very few fires started near Melbourne which is located at the middle of
Victoria.
...
\hypertarget{paremeter-tuning}{%
\subsubsection{Paremeter tuning}\label{paremeter-tuning}}
