...
observations of a bbushfire and the temporal dimension needs to be
treated differently in the clustering algorithm.ushfire in several
hours. It suggests that hotspots with long intervals may belong to the
same One possible solution to this issue is dividing hotspot data into
intervals. In other words, the temporal dependence between hotspots is
predetermined by a parameter \(ActiveTime\). The interpretation of
\(ActiveTime\) is the time a fire can stay smouldering but undetectable
by satellite before flaring up again.
