...
The clustering result obtained from the `hotspot_cluster()` function is a `spotoroo` object which is not handful to be used in further analysis. To overcome this issue, the package provides a function `extract_fire()` to convert a `spotoroo` object to a `data.frame`. 
...
The default plot produced by the function shown in Figure \ref{fig:demodefplot} is a scatter plot of the clusters and their ignition locations showing the spatial distribution of the fires. 
...
Figure \ref{fig:demomovplot} shows the path of the fire movement, which can be produced by setting `type = 'mov'`. The argument `step` controls the time difference between successive step and using small value of `step` will produce complex path. Moreover, the fire movement is computed by the `get_fire_mov()` function.
...
Figure \ref{fig:demotimeline} shows the time line of clusters, which can be produced by setting `type = 'timeline'` to study the intensity of the bushfire season. In this plot, the green dots are hotspots belong to different clusters and the green wave is the density plot for all the hotspots. Besides, the orange dots are noise. From this plot, we can say that most of the hotspots were observed in the mid-January.
...
To perform the clustering algorithm on the Himawari-8 hotspot data, we first transform the observed time to time index by setting the time difference between two successive index to be $1$ hour. Then, set `activeTime` to be $24$ time indexes, `adjDist` to be $3000$ memters, `minPts` to be $3$ hotspots and `minTime` to be $3$ indexes for the algorithm. The choice of these parameters will be justified in the section [Paremeter tuning].
...
The result is given in Figure \ref{fig:clusteringfinalresults}. From this plot, we observe that most of the fires were ignited in the east of Victoria and some fires were ignited in the south west of the Victoria. Very few fires started near Melbourne which is located at the middle of Victoria.
