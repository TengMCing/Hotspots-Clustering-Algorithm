...
In this section, we will describe the implementation of the algorithm in the R package \CRANpkg{spotoroo}. You can install the released version from CRAN.
...
In this function, three different kinds of arguments need to be specified. Arguments `hotspots`, `lon`, `lat` and `obsTime` are used to specify the data set and its relevant columns. Arguments `activeTime`, `adjDist`, `minPts` and `minTime` have already been defined in the [Algorithm] section. Besides, arguments `timeUnit` and `timestep` are used to convert observed time to discrete time index.
...
The output of this function is a \CRANpkg{spotoroo} object, which is actually a `list` contains a `data.frame` called `hotspots`, a `data.frame` called `ignition` and a `list` called `setting`.
...
Use the generic function `summary` to get a brief summary of the clustering result.
...
To keep all information from the clustering result including noise points, use `extract_fire()`. It will convert a \CRANpkg{spotoroo} object to a `data.frame`.
...
The "def" plot produces a scatter plot of the clusters and their ignition locations. It indicates the spatial distribution of the fires.

```{r echo=TRUE}
plot(result, type = "def")
```

The "mov" plot produces paths of the movement of the centroid of the clusters. The argument `step` controls the time difference between each successive step. The movement can be obtained from the `get_fire_mov()` function.

```{r echo=TRUE}
plot(result, type = "mov", step = 6)
```

The "timeline" plot produces a timeline for the observed hotpots. It is useful in inspecting the bushfire season.
