...
Hostpots will first be divided into hours as shown in Figure \ref{fig:step1}. Notice the unit of time is an arbitrary choice. Theoretically, it can be replaced with any other units not larger than the total length of time and not less than the temporal resolution of the data. It depends on 

```{r step1, fig.cap="Hotspots in the first 3 hours."}
ggplot(filter(hotspots, hour_id <= 5)) +
  geom_sf(data = vic_map) +
  geom_point(aes(lon, lat), position = "jitter") +
...
We try to formalize the meaning of "cluster" to better illustrate the connection between 
