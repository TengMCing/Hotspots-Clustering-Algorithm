...
```{r step2figs, fig.cap="Description", fig.height=6, fig.width = 8}
...
```{r}
ggplot() +
  geom_text(data = points_2[1:20, ], aes(x, y, col = paste0("Cluster ", as.character(point_cols2[1:20])), label = point_cols)) +
  geom_point(data = points_2[21:30, ], aes(x, y, col = paste0("Cluster ", as.character(point_cols2[21:30])))) +
  coord_fixed() +
  theme_bw() +
  theme(legend.position = "left") +
  theme(axis.line=element_blank(),
      ...
      axis.title.x=element_blank(),
      axis.title.y=element_blank()) +
  labs(col = "", shape = expression("Hotspots "*" in "*S[t-1]), title = "(a)", subtitle = quote(S[t])) +
  scale_shape_manual(values = c(1, 2)) -> p3
```



```{r step3figs, fig.height=6, fig.width = 8, fig.cap = "Description"}
ggplot() +
  ...
  theme(axis.line=element_blank(),
      ...
      axis.title.x=element_blank(),
      axis.title.y=element_blank()) +
  labs(col = expression("Hotspots "*" in "*S[t-1]), title = "(b)", subtitle = quote(S[t])) +
  scale_shape_manual(values = c(1, 2)) -> p4
